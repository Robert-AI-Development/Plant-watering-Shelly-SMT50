...
if (obj(K.day) && K.day.n >= c3.maxDay) K.day = { date: null, n: 0, sec: 0 };
K.job = { ok: true, sec: h.pumpSec, pct: null, why: "hwtest", ts: now };
run0 = now;
...
function stepReport() {
if (!passB && res.p0 === "ok" && res.p1 !== "fe") return true;
rep = [];
...
ri = 0;
if (!passB) { writeHwp(abort ? "abbruch" : "ende", onRepWritten); return; }
mode = "rep";
}
