...
Wasserstand-Schwimmer). Drei Scripts laufen **auf dem Gerät** in der Shelly-Script-Engine (mJS); `tools/` enthält
...
- `npm test` (58) und `npm run check` grün; `npm run build` erzeugt `dist/` unter dem Größenlimit.
...
Three scripts run **on the device** in the Shelly script engine (mJS); `tools/` holds a Node mock so everything is
...
keep the version comment + `var VER`.

### Definition of done

`npm test` (58) and `npm run check` green; `npm run build` produces `dist/` under the size limit; docs updated when
