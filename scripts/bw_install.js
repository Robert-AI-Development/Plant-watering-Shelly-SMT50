// bw_install.js v0.1.0 – Installer: KVS-Startwerte, Zeitplan, Script- und Switch-Konfiguration (einmalig von Hand starten)
//
...
// Vorhandene KVS-Einträge werden nie überschrieben. Der Installer ist beliebig oft wiederholbar:
...
function failErr(msg) {
  ...
  Shelly.call("KVS.Set", { key: "err", value: e }, onFailWritten);
}
...
function next() {
  ...
  var f = steps[si];
  si = si + 1;
  try { f(); } catch (e) { fail("Ausnahme in Schritt " + si + ": " + (e && e.message ? e.message : e)); }
}
...
function stepScripts() {
  ...
  Shelly.call("Script.List", {}, onScriptList);
}
...
function stepSchedList() { Shelly.call("Schedule.List", {}, onSchedList); }
...
function stepSchedDelete() { di = 0; delNext(); }
function delNext() {
  if (di >= delq.length) { next(); return; }
  var id = delq[di];
  di = di + 1;
  Shelly.call("Schedule.Delete", { id: id }, onDel, id);
}
...
function createNext() {
  if (ci >= crq.length) { next(); return; }
  var e = crq[ci];
  ci = ci + 1;
  Shelly.call("Schedule.Create", { enable: true, timespec: e.timespec, calls: e.calls }, onCreate, e.timespec);
}
...
function stepScriptCfg() { scq = [ids.main, ids.pump]; sci = 0; scNext(); }
function scNext() {
  if (sci >= scq.length) { next(); return; }
  var id = scq[sci];
  sci = sci + 1;
  Shelly.call("Script.SetConfig", { id: id, config: { enable: false } }, onScriptCfg, id);
}
...
function stepSwitchCfg() {
  ...
  Shelly.call("Switch.SetConfig", { id: K.cfg1.idSw, config: cfg }, onSwitchCfg);
}
