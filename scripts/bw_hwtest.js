...
// Kommandos vom Menschen/Werkzeug über KVS hwc {n, cmd: go|skip|abort}; Stand und Bericht in hwr; Schwellen/Zeiten in hwt.
// Kalibrierpunkte vDry/vWet/lvlEmpty werden bei Plausibilität in cfg1 geschrieben (hwt.cal = 1). Der Pumpentest ist bw_hwpump.js.
// Ergebniscodes je Phase: ok sk (übersprungen) to (Timeout) ab (Abbruch) nl (Sensor liefert null)

var VER = "0.1.0";
var DEBUG = 0;                 // 1 = Debug-Zeilen: Schritte, RPC-Aufrufe, KVS-Inhalt
var REQ1 = ["vDry", "vWet", "vErrLo", "vErrHi", "lvlEmpty", "nLvl", "idV", "idT", "idLvl", "idSw"];
// Startwerte für hwt (nur angelegt, wenn der Eintrag fehlt); die Pumpenfelder nutzt bw_hwpump
...
function note(v) {
  // Skalare und ein Ring von nStab Werten – kein wachsendes Array über 1800 Ticks
  p.last = v;
  if (p.min === null || v < p.min) p.min = v;
  if (p.max === null || v > p.max) p.max = v;
  p.ring[p.ri % h.nStab] = v;   // Ringpuffer per Index: mJS kennt kein Array.shift (LEARNING.md)
  p.ri = p.ri + 1;
}
function ringAt(k) { return p.ring[(p.ri - 1 - k) % h.nStab]; }   // k = 0 jüngster Wert
...
// Prüffunktionen: null = weiter warten, sonst Ergebniscode
...
// Phasentabelle in einer Funktion (kein mehrzeiliges Literal auf Modulebene: mJS hoistet nicht)
function phases() {
  return [
...
// ---- Schritt 2: Konfiguration, Komponenten, Timer -------------------------------------
function stepCfg() {
  c1 = K.cfg1;
...
// ---- Phase starten (ein Schritt je Phase, Reihenfolge aus PH) --------------------------
function stepPhase() {
  pi = pi + 1;
  ph = PH[pi];
  if (pi === 0) { K = {}; orig = {}; }   // Speicher freigeben: der Script-Heap (~25 KB) ist mit bw_main geteilt (LEARNING.md); stepReread liest neu
  if (h.run.indexOf(ph.g) < 0) { res[ph.id] = "sk"; writeHwr("lauf", onPhaseDone); return; }   // Stand auch bei Überspringen sichtbar
  p = newPhase();
  mode = "ph";
  log("Phase " + (pi + 1) + "/" + PH.length + " " + ph.id + ": " + ph.txt + " (Timeout " + h.tPhase + " s)");
}

// ---- Kalibrierwerte prüfen, KVS neu lesen, cfg1 + Bericht schreiben ---------------------------
function stepCal() {
  mode = "idle";
  var why = [];
...
function stepWrite() {
  // cfg1 frisch gelesen (Lesen-Ändern-Schreiben aller Felder), nur bei Änderung schreiben
