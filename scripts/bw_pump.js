...
//
// Schrittkette (flache next()-Schleife): 1 KVS.Get je Schlüssel  2 Pflichtfelder/Uhr  3 Auftrag, Guard, Frist, Kopien, K freigeben
// 4 Fenster als Tick-Automat (ein Timer, ein offener RPC)  5 KVS neu lesen  6 Ergebnis  7 schreiben  8 Konsole, Script.Stop.
// Automat (mode): m0 messen → lv Wasserstand → cl Claim → on einschalten → pu pumpen → so einsickern → st stabilisieren → decide()
// → lv (nächste Portion) oder done. end(why) setzt nur mode="done"; der nächste Tick räumt den Timer weg und ruft next().
// Alle Abbrüche (auch vor dem Fenster) laufen über das Neu-Lesen: errSet wird dort gesetzt, der Auftrag verbraucht. Der Tageswechsel
// von day ist Sache von bw_main (läuft vor dem Fenster); bw_pump braucht kein Datum. Zeit im Fenster aus Shelly.getUptimeMs().

var VER = "0.2.0";
var DEBUG = 0;                 // 1 = Debug-Zeilen in der Konsole: Schritte, RPC-Aufrufe, Messwerte
...
var now = 0, ram = null;
var j = null, l = null, dn = 0, ds = 0;   // Kopien: Auftrag, Lernwerte, Tageszähler
var B = 0, t0 = 0;                        // Frist in s ab Scriptstart, Uptime-ms beim Start
...
var stab = { t0: 0, at: 0 };
var soak0 = 0, chk0 = 0, sec1 = 0, tMeas = 0;   // tMeas = tSoak + nStab·tStep
...
// genau ein offener Shelly.call: Callback in Modulvariablen, busy sperrt den Tick
...
// Median der Proben (robust gegen Ausreißer); null bei fehlender oder unplausibler Spannung
function pctOf(arr) {
...
function ringAt(k) { return ring[(ri - 1 - k) % c4.nStab]; }   // k = 0 jüngster Wert
...
// ---- Schritt 1: KVS.Get je Schlüssel (keine fremden Einträge im Heap) --------
...
// ---- Schritt 2: Pflichtfelder und Uhr ------------------------------------------
function stepCfg() {
...
// ---- Schritt 3: Auftrag prüfen, Frist, Kopien, K freigeben ---------------------
function stepCheck() {
  ...
  var q = now % (c3.tick * 60);   // Sekunden seit dem letzten bw_main-Takt (Zeitzonenversatz ist ein Vielfaches von 15 min, tick teilt 15)
...
// ---- Schritt 4: Fenster als Tick-Automat ---------------------------------------
function stepWin() {
...
// ---- Schritt 5: KVS neu lesen (bw_main kann bis zum Takt geschrieben haben) ----
function stepReread() { K = {}; keys = RE; gi = 0; getNext(); }

// ---- Schritt 6: Ergebnis nach Matrix ----------------------------------------------
function stepResult() {
  readClock();
...
// ---- Schritt 7: schreiben, nur Änderungen -------------------------------------------
function stepWrite() {
  wq = [];
