// bw_pump.js v0.1.0 – Gießfenster (08:00/20:00): Auftrag prüfen, Pumpe schalten, Wasserstand überwachen, Ergebnis schreiben
//
...
function pumpOffSafe() {
  ...
  if (c1 && typeof c1.idSw === "number") Shelly.call("Switch.Set", { id: c1.idSw, on: false }, onFailOff);
  else stop();
}
...
function finish(why) { out.why = why; jumpTo(stepWrite); }
...
function abortErr(code, msg) { log("Störung " + code + ": " + msg); setErr(code); finish(code); }
function readClock() {
...
function stepCfg() {
  ...
  if (!(c1.nLvl >= 1) || !(c1.msSample >= 1) || !(c3.tChk >= 1)) { abortErr("cfg", "cfg1.nLvl/msSample und cfg3.tChk müssen ≥ 1 sein"); return; }
  if (!readClock()) { abortErr("uhr", "Uhrzeit nicht gesetzt"); return; }
  next();
}
...
function stepCheck() {
  job = K.job;
  if (!job || typeof job !== "object" || job.ok !== true) { log("kein Auftrag (job.ok=false, why=" + (job ? job.why : "-") + ")"); finish("kein_auftrag"); return; }
  if (!isNum(job.ts) || now - job.ts > c3.jobAge * 60) { abortErr("alt", "Auftrag ist " + (isNum(job.ts) ? Math.round((now - job.ts) / 60) : "?") + " min alt, erlaubt " + c3.jobAge); return; }
  var blk = blockCode();
  if (blk !== null) { log("Störung " + blk + " steht, kein Gießen"); finish("err:" + blk); return; }
  if (K.day.date !== today) K.day = { date: today, n: 0, sec: 0 };
  if (K.day.n >= c3.maxDay) { abortErr("limit", "Tageslimit " + c3.maxDay + " erreicht"); return; }
  if (!isNum(job.sec) || job.sec < 1) { log("Auftrag ohne gültige Sekunden"); finish("kein_auftrag"); return; }
  next();
}
...
function onLevel() {
  try {
    ...
    if (!stable) { log("Wasserstand unstabil, kein Gießen"); finish("lvl"); return; }
    if (lvl.s[0] === true) { abortErr("wasser", "Behälter leer"); return; }
    next();
...
function stepPump() {
  ...
  Shelly.call("Switch.Set", { id: c1.idSw, on: true, toggle_after: run.sec }, onSwitchOn);
}
function onSwitchOn(res, ec, em) {
  if (ec !== 0) { log("Switch.Set ein fehlgeschlagen: " + em); finish("switch"); return; }
...
function endRun(result, actual) {
  ...
  Shelly.call("Switch.Set", { id: c1.idSw, on: false }, onSwitchOff);
}
...
function stepReread() { K = {}; si = si - 1; steps[si] = stepReadAgain; next(); }
function stepReadAgain() { kvsPage(0); }
